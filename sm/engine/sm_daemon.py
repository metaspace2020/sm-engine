...
class SMAnnotateDaemon(object):
    ...
    def __init__(self, manager, annot_qdesc, index_upd_qdesc, poll_interval=1):
        self._sm_config = SMConfig.get_conf()
        self._stopped = False
        ...
        self._index_upd_queue_pub = QueuePublisher(config=self._sm_config['rabbitmq'],
                                                   qdesc=index_upd_qdesc,
                                                   logger=self.logger)
    ...
    def _callback(self, msg):
        ...
        self._index_upd_queue_pub.publish(msg=upd_msg, priority=2)
...
class SMUpdateDaemon(object):
    ...
    def _callback(self, msg):
        ...
        self.logger.info(log_msg)
        ds = Dataset.load(self._db, msg['ds_id'])
        if ds.status in {DatasetStatus.STARTED, DatasetStatus.INDEXING}:
