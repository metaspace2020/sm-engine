...
class SMapiDatasetManager(object):
    ...
    def __init__(self, db, es, image_store, logger=None,
                 annot_queue=None, ind_update_queue=None, status_queue=None):
        ...
        self._ind_update_queue = ind_update_queue
        self.logger = logger
    ...
    def _post_sm_msg(self, ds, queue, priority=DatasetActionPriority.DEFAULT, **kwargs):
        ...
        self.logger.info('New message posted to %s: %s', self._annot_queue, msg)

    def add(self, ds, del_first=False):
        """ Send add message to the queue """
        self._post_sm_msg(ds=ds, queue=self._annot_queue, del_first=del_first)

    def delete(self, ds, del_raw_data=False):
        """ Send delete message to the queue """
        self._post_sm_msg(ds=ds, queue=self._ind_update_queue, action='delete')

    def update(self, ds):
        """ Send index message to the index update queue """
        self._post_sm_msg(ds=ds, queue=self._ind_update_queue, action='update')
